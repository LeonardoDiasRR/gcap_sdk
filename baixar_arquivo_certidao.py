...
# -*- coding: utf-8 -*-
"""
Script para baixar arquivo PDF de Certidão do storage S3.
Recebe como parâmetro um número de mandado de prisão, pesquisa os dados 
do mandado utilizando gcap_sdk e faz download do arquivo PDF da certidão.
"""
...
def gerar_nome_arquivo(numero_mandado, url_arquivo):
    ...
    # Criar nome formatado: certidao_<numero_normalizado>_<timestamp>
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    nome_arquivo = f"certidao_{numero_mandado}_{timestamp}{extensao}"
    
    return nome_arquivo


def baixar_arquivo_certidao(numero_mandado):
    """
    Baixa arquivo PDF de Certidão do S3.
    ...
    try:
        ...
        # Procurar pelo arquivo de certidão
        # Tenta: arquivo_certidao, arquivo_certidões, certidao_arquivo, etc.
        campos_certidao = [
            'arquivo_certidao',
            'arquivo_certidões',
            'certidao_arquivo',
            'arquivo_mandado'  # Fallback mencionado pelo usuário
        ]
        
        caminho_arquivo = None
        for campo in campos_certidao:
            if campo in mandado and mandado[campo]:
                caminho_arquivo = mandado[campo]
                break
        
        if not caminho_arquivo:
            # Listar campos disponíveis para debug
            campos_disponiveis = list(mandado.keys())
            return {
                'success': False,
                'error': "O mandado não possui arquivo de certidão. Campos disponíveis: " + 
                         ", ".join(campos_disponiveis[:10]) + ("..." if len(campos_disponiveis) > 10 else ""),
                'debug_campos': campos_disponiveis
            }
        ...
        try:
            ...
            return {
                'success': True,
                'message': "Arquivo de certidão baixado com sucesso",
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Uso: python baixar_arquivo_certidao.py <numero_mandado>")
        print("Exemplos:")
        print("  - Formatado: python baixar_arquivo_certidao.py 0841162-22.2025.8.23.0010.01.0001-17")
        print("  - Somente números: python baixar_arquivo_certidao.py 0841162222025823001001000117")
        sys.exit(1)
    ...
    result = baixar_arquivo_certidao(numero_ou_erro)
